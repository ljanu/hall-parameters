<?php

return [
    "routes" => array(

        //hala-parametry
        "h_p_index" => "h-p-cs.index",
        "h_p_show"  => "h-p-cs.show",

        // construction
        "construction_create" => "konstrukce.create",
        "construction_edit"   => "konstrukce.edit",

        //specification
        "specification_create" => "specifikace.create",
        "specification_edit" => "specifikace.edit",

    ),
    "text"   => array(

        // base text
        "list"                    => "seznam",
        "new"                     => "nové zadání",
        "edit"                    => "upravit",
        "save"                    => "uložit",
        "remove"                  => "odstranit",
        "add"                     => "odstranit",
        "yes"                     => "ano",
        "no"                      => "ne",
        "partly"                  => "částečně",

        //project
        "project"                 => "projekt",
        "new_project"             => "nový projekt",
        "edit_project"            => "upravit projekt",
        "list_projects"           => "seznam projeků",

        // construction
        "construction_parameters" => "parametry konstrukce",
        "dimension"               => "rozměry",
        "width"                   => "šířka",
        "length"                  => "délka",
        "height"                  => "výška",
        "roof_type"               => "typ střechy",
        "gable_roof"              => "sedlová střecha",
        "flat_roof_left"          => "pultová střecha se sklonem do leva",
        "flat_roof_right"         => "putlová střecha se sklonem do prava",
        "limitations"             => "omezení",
        "roof_pitch"              => "sklon střechy",
        "max_module_size"         => "max vzdálenost modulu",
        "max_purlin_size"         => "max vzdálenost vaznic",
        "max_side_rails_size"     => "max vzdálenost vazniček",

        //specification
        "new_specification"            => "nová specifikace",
        "edit_specification"           => "upravit specifikaci",
        "form_metadata"                => "form metadata",
        "specification"                => 'specifikace',
        "name_object"                  => "název objektu",
        "roof_load"                    => "zatížení střechy",
        "load"                         => "zatížení",
        "insulated"                    => "zateplení",
        "basic_color_design"           => "základní barevné provedení",
        "color_roof"                   => "barva střechy",
        "color_walls"                  => "barva stěn",
        "color_fill"                   => "barva doplňků",
        "color_edges"                  => "barva lemování",
        "color_construction"           => "barva konstrukce",
        "cladding_orientation"         => "orientace kladení pláště",
        "foundation_build"             => "založení objektu",
        "height_floor"                 => "výška podlahy",
        "bottom_edge_facade"           => "spodní hrana pláště",
        "fire_resistance"              => "požární odolnost",
        "fire_resistance_construction" => "požární odolnost konstrukce",
        "fire_resistance_roof"         => "požární odolnost střechy",
        "fire_resistance_walls"        => "požární odolnost stěn",
    ),
];
